...
public class Transfer extends Transaction {
	...
	protected Message getSpecificsFromCustomer() throws Cancelled {
		from = ATMPanel
				.getInstance()
				.getConsole()
				.readMenuChoice("Account to transfer from",
						AccountInformation.ACCOUNT_NAMES);

		to = ATMPanel
				.getInstance()
				.getConsole()
				.readMenuChoice("Account to transfer to",
						AccountInformation.ACCOUNT_NAMES);

		amount = ATMPanel.getInstance().getConsole()
				.readAmount("Amount to transfer");

		return new Message(Message.TRANSFER, card, pin, serialNumber, from, to,
				amount);
	}
