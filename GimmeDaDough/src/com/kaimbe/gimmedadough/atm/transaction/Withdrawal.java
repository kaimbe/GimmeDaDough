...
public class Withdrawal extends Transaction {
	...
	protected Message getSpecificsFromCustomer() throws Cancelled {
		from = ATMPanel
				.getInstance()
				.getConsole()
				.readMenuChoice("Account to withdraw from",
						AccountInformation.ACCOUNT_NAMES);

		String[] amountOptions = { "$20", "$40", "$60", "$100", "$200" };
		Money[] amountValues = { Money.parse("CAD 20"), Money.parse("CAD 40"),
				Money.parse("CAD 60"), Money.parse("CAD 80"),
				Money.parse("CAD 100") };
		...
		while (!validAmount) {
			amount = amountValues[ATMPanel
					.getInstance()
					.getConsole()
					.readMenuChoice(
							amountMessage + "Amount of cash to withdraw",
							amountOptions)];

			validAmount = ATMPanel.getInstance().getCashDispenser()
		...
		return new Message(Message.WITHDRAWAL, card, pin, serialNumber, from,
				-1, amount);
