...
public class Deposit extends Transaction {
	...
	protected Message getSpecificsFromCustomer() throws Cancelled {
		to = ATMPanel
				.getInstance()
				.getConsole()
				.readMenuChoice("Account to deposit to",
						AccountInformation.ACCOUNT_NAMES);

		amount = ATMPanel.getInstance().getConsole()
				.readAmount("Amount to deposit");

		return new Message(Message.INITIATE_DEPOSIT, card, pin, serialNumber,
				-1, to, amount);
	}
	...
	protected Receipt completeTransaction() throws Cancelled {
		...
		ATMPanel.getInstance()
				...
				.sendMessage(
						new Message(Message.COMPLETE_DEPOSIT, card, pin,
								serialNumber, -1, to, amount), balances);
